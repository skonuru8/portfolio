...
export function HeroClient({ pdfReady }: { pdfReady: boolean }) {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 motion-gradient opacity-90"
        style={{
          backgroundImage:
            "linear-gradient(125deg, rgba(2,6,23,1) 0%, rgba(15,23,42,0.95) 35%, rgba(30,58,138,0.35) 55%, rgba(8,47,73,0.5) 75%, rgba(2,6,23,1) 100%)",
        }}
        aria-hidden
      />
      ...
      <ParallaxLayer offset={72} className="z-10 mx-auto max-w-6xl px-4 md:px-6">
        <p className="font-mono-label text-xs uppercase tracking-[0.28em] text-signal">
          From noise to{" "}
          <span className="hero-signal" data-in-view={inView ? "true" : "false"}>
            signal
          </span>
        </p>
