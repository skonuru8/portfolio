  ...
  return (
    <Section id="thinking" ariaLabel="How I think">
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        ...
        <ol className="mb-12 hidden max-w-3xl space-y-3 md:block">
          {thinkingSequence.map((line, i) => (
            <Reveal key={line} delay={i * 0.04}>
              <li className="flex gap-4 border-l border-line pl-4">
                <span className="font-mono-label text-xs text-signal">{String(i + 1).padStart(2, "0")}</span>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {cards.map((c, i) => (
            <Reveal key={c.title} delay={i * 0.06}>
              <article className="h-full rounded-xl border border-line bg-panel/60 p-5">
